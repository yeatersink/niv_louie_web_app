...
def startup():
    """Run once when the server starts"""
    try:
        # === Copy static files from host to container at runtime (reliable fix) ===
        static_src = "/home/yeatersink/niv-louie-app/static"
        ...
        if os.path.exists(static_src):
            ...
            print(f"LOG: Successfully copied static files to {static_dest}")
        else:
            print(f"WARNING: Static folder not found on host at {static_src}")

        # Register static files with NiceGUI
        app.add_static_files('/static', '/app/static')
