...
def _static_directory():
    repo_static = Path(__file__).resolve().parent / "static"
...
def startup():
    ...
    try:
        ...
        if static_dir.is_dir():
            ...
            print(f"LOG: Serving static files from {static_dir}")
        elif Path("/app/static").is_dir():
            app.add_static_files("/static", "/app/static")
            print("LOG: Serving static files from /app/static")
        else:
            print(f"WARNING: Directory '{static_dir}' does not exist")
